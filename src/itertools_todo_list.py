...
sorted_tasks = sorted(tasks, key=lambda x: (x[0], int(x[-1])))
groupped_tasks = groupby(sorted_tasks, key=lambda x: x[0])
for keys, values in groupped_tasks:
    print(keys)
    for value in values:
        print(f'\t{value[-1]}. {str.capitalize(value[-2])}.')
