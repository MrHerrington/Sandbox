...
groupped = list((key, list(val)) for key, val in groupby(sorted(students, key=sort_key), key=sort_key))
print(sorted(groupped, key=lambda y: len(y[1]), reverse=True)[0][0])